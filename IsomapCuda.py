...
def KNNKernel(targetChunkSize,options,prefix = ''):
    ...
    program =  ("__global__ void "+prefix+"knn(const float* source, const float* target, float* distances, unsigned int* indices, const long target_offset, const long source_offset) {\n"+
                ...
                "        if (distance <= distances[source_address+"+str(options['k'])+"-1] and distance < "+str(options['eps'])+" and (i+offset) != (v+offset2)) {\n"+
                "            unsigned int j = 0;\n"+
...
def KNN(dataTable,knnOptions):
    ...
    for t in xrange(len(data)-1):
        ...
        for source in data[:-1]:
            ...
            offset2 += 1
        prg(drv.In(data[-1]),drv.In(data[t]),drv.InOut(dists[offset2]),drv.InOut(indices[offset2]),numpy.int64(t*knnOptions['chunkSize']),numpy.int64(offset2*knnOptions['chunkSize']),block=(knnOptions['lastChunkSize'],1,1))
    ...
    for source in data[:-1]:
        ...
        offset2 += 1
    prg2(drv.In(data[-1]),drv.In(data[t]),drv.InOut(dists[offset2]),drv.InOut(indices[offset2]),numpy.int64(t*knnOptions['chunkSize']),numpy.int64(offset2*knnOptions['chunkSize']),block=(knnOptions['lastChunkSize'],1,1))
    ...
    print len(allindices),knnOptions['totalChunks']
